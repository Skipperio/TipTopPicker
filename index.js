...
//DownTimes
let downtime = [];
// changes level of and values of palletOnTheFloor cageOnTheFloor totalOnTheFloor
...
function addfromCages() {
  // adds cases from cages to picked
...
function nextQuestionButton() {
  // hide next question button because there is no another question
  if (currentQuestion == listOfQuestions.length) {
    console.log("to hide next button " + currentQuestion);
    document.getElementById("newNextButton").style.display = "none";
  }
  currentQuestion++;
...
function previousQuestionButton() {
  // to show next question button because there is previous question
  if (currentQuestion <= listOfQuestions.length) {
    console.log("to hide next button " + currentQuestion);
    document.getElementById("newNextButton").style.display = "block";
  }

  // check 0. question mean "zeroid" question not to hide it
...
function showQuestion() {
  ...
  if (currentQuestion < listOfQuestions.length) {
    document.getElementById(listOfQuestions[previousQuestion]).style.display =
      "none";
  }
}
